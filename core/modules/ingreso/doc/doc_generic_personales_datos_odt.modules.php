...
/**
 *	\file       htdocs/core/modules/ingreso/doc/doc_generic_personales_datos_odt.modules.php
 *	\ingroup    ingreso
 *	\brief      File of class to build ODT documents for personales_datoss
 */

dol_include_once('/ingreso/core/modules/ingreso/modules_personales_datos.php');
 ...
 */
class doc_generic_personales_datos_odt extends ModelePDFPersonales_Datos
{
	/**
	 * Issuer
	 * @var Societe
	...
	public function __construct($db)
	{
		...
		$this->scandir = 'INGRESO_MYOBJECT_ADDON_PDF_ODT_PATH'; // Name of constant that is used to save list of directories to scan
	...
	public function info($langs)
	{
		...
		$odtPath = trim(getDolGlobalString('INGRESO_MYOBJECT_ADDON_PDF_ODT_PATH'));
		...
		$texte .= '<input type="hidden" name="param1" value="INGRESO_MYOBJECT_ADDON_PDF_ODT_PATH">';
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=ingreso_personales_datos/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=doctemplates&keyforuploaddir=INGRESO_MYOBJECT_ADDON_PDF_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picto('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
		...
		if (!getDolGlobalString('MAIN_NO_MULTIDIR_FOR_ODT')) {
			...
			$texte .= '<br>';
			$texte .= '<input type="hidden" name="value1" value="INGRESO_MYOBJECT_ADDON_PDF_ODT_PATH">';
		}
		...
		$texte .= '<input type="hidden" value="INGRESO_MYOBJECT_ADDON_PDF_ODT_PATH" name="keyforuploaddir">';
	 ...
	 *
	 *  @param	Personales_Datos	$object				Source object to generate document from
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->ingreso->dir_output) {
			...
			$dir = $conf->ingreso->multidir_output[isset($object->entity) ? $object->entity : 1];
			...
			if (file_exists($dir)) {
				...
				dol_mkdir($conf->ingreso->dir_temp);
				if (!is_writable($conf->ingreso->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $conf->ingreso->dir_temp);
				...
				// Line of free text
				$newfreetext = '';
				$paramfreetext = 'INGRESO_MYOBJECT_FREE_TEXT';
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
							'PATH_TO_TMP'	  => $conf->ingreso->dir_temp,
