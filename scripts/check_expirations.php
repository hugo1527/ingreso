 ...
 *
 * NOTA: Este script es llamado por cron_run_jobs.php (Tipo: Método de llamada de clase).
 * NO debe incluir main.inc.php, ya que el entorno se carga antes.
 */

// ¡No incluir main.inc.php aquí!
...
class IngresoScheduledTasks
{
     ...
     * Las variables $db, $conf, $langs serán inyectadas por Dolibarr si se configura la tarea.
     * O las cargaremos globalmente en el método.
     */
    public function __construct($db = null)
    {
        // Si Dolibarr nos pasa la $db (versiones > 16), la usamos.
        // Si no, la tomaremos de 'global' en checkAllExpirations.
        if ($db) {
            $this->db = $db;
        }
    }
    ...
    {
        global $user; // $user global de main.inc.php
        
        // Buscamos el primer usuario superadmin
    ...
    {
        global $db, $conf, $langs, $user; // ¡IMPORTANTE! Obtenemos las variables globales
...
// --- FIN DE EJECUCIÓN DEL SCRIPT ---
// (No debe haber nada aquí abajo)