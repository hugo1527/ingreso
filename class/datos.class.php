...
/**
 * \file        class/datos.class.php
 * \ingroup     ingreso
 * \brief       This file is a CRUD class file for Datos (Create/Read/Update/Delete)
 */
 ...
 * Class for Datos
 */
class Datos extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'datos';

	/**
	 * @var string 		Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'ingreso_datos';

	/**
	 * @var string 		If permission must be checkec with hasRight('ingreso', 'read') and not hasright('mymodyle', 'datos', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'ingreso';

	/**
	 * @var string 		String with name of icon for datos. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'datos@ingreso' if picto is file 'img/object_datos.png'.
	 */
	...
	public $fields = array(
		...
		"ref" => array("type" => "varchar(128)", "label" => "ID_Ingreso", "enabled" => "1", 'position' => 20, 'notnull' => 1, "visible" => "1", "index" => "1", "searchall" => "1", "validate" => "1", "comment" => "Reference of object"),
		...
		"apellido" => array("type" => "varchar(128)", "label" => "Apellido", "enabled" => "1", 'position' => 31, 'notnull' => 1, "visible" => "1", "searchall" => "1",),
		"nombre" => array("type" => "varchar(255)", "label" => "Nombre", "enabled" => "1", 'position' => 33, 'notnull' => 1, "visible" => "1", "searchall" => "1",),
		"phone" => array("type" => "varchar(128)", "label" => "Telefono", "enabled" => "1", 'position' => 35, 'notnull' => 0, "visible" => "1", "searchall" => "1",),
		"mail" => array("type" => "email", "label" => "Email", "enabled" => "1", 'position' => 37, 'notnull' => 0, "visible" => "1", "searchall" => "1",),
		"alternative_phone" => array("type" => "varchar(128)", "label" => "Telefono Alternativo", "enabled" => "1", 'position' => 39, 'notnull' => 0, "visible" => "1", "searchall" => "1",),
	);
	public $rowid;
	public $ref;
	...
	//  */
	// public $table_element_line = 'ingreso_datosline';
	...
	//  */
	// public $fk_element = 'fk_datos';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Datosline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Datos', 'fk_element'=>'fk_datos'));
	...
	//  */
	// protected $childtablesoncascade = array('ingreso_datosdet');

	// /**
	//  * @var DatosLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('ingreso', 'datos', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('ingreso', 'datos', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('ingreso', 'datos_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'datos/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'datos/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filepath = 'datos/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'datos/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->ingreso->dir_output.'/datos/'.$oldref;
				$dirdest = $conf->ingreso->dir_output.'/datos/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->ingreso->dir_output.'/datos/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowDatos")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Datos").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/ingreso/datos_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$objectline = new DatosLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_datos:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('INGRESO_MYOBJECT_ADDON')) {
			$conf->global->INGRESO_MYOBJECT_ADDON = 'mod_datos_standard';
		}
		...
		if (getDolGlobalString('INGRESO_MYOBJECT_ADDON')) {
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefDatos $obj';
	...
	/**
     * Método para copiar propiedades comunes a objetos hijos
     */
    public function copyCommonPropertiesTo(&$targetObject)
    {
        $targetObject->apellido = $this->apellido;
        $targetObject->nombre = $this->nombre;
        $targetObject->phone = $this->phone;
        $targetObject->mail = $this->mail;
        $targetObject->alternative_phone = $this->alternative_phone;
        $targetObject->status = $this->status;
        // Copia otros campos comunes si es necesario
    }

    /**
     * Método para inicializar desde un objeto hijo (si es necesario)
     */
    public function initFromChildObject($childObject)
    {
        $this->apellido = $childObject->apellido;
        $this->nombre = $childObject->nombre;
        $this->phone = $childObject->phone;
        $this->mail = $childObject->mail;
        $this->alternative_phone = $childObject->alternative_phone;
        $this->status = $childObject->status;
    }
}
...
/**
 * Class DatosLine. You can also remove this and generate a CRUD class for lines objects.
 */
class DatosLine extends CommonObjectLine
{
	// To complete with content of an object DatosLine
	// We should have a field rowid, fk_datos and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'datos'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_datos'
