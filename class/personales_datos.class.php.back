...
/**
 * \file        class/personales_datos.class.php
 * \ingroup     ingreso
 * \brief       This file is a CRUD class file for Personales_Datos (Create/Read/Update/Delete)
 */

// Put here all includes required by your class file
require_once DOL_DOCUMENT_ROOT.'/core/class/commonobject.class.php';
require_once DOL_DOCUMENT_ROOT.'/custom/ingreso/class/datos.class.php';
...
//class Personales_Datos extends Datos  // CAMBIO AQUÍ: extends Datos en lugar de CommonObject
{
	 ...
	 */
	public $module = 'ingreso';

	/**
	 * @var string 		ID to identify managed object.
	 */
	public $element = 'personales_datos';

	/**
	 * @var string 		Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'ingreso_personales_datos';

	/**
	 * @var string 		If permission must be checkec with hasRight('ingreso', 'read') and not hasright('mymodyle', 'personales_datos', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'ingreso';

	/**
	 * @var string 		String with name of icon for personales_datos. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'personales_datos@ingreso' if picto is file 'img/object_personales_datos.png'.
	 */
	...
	public $fields = array(
		...
		"tms" => array("type" => "timestamp", "label" => "DateModification", "enabled" => "1", 'position' => 502, 'notnull' => 0, "visible" => "0", "default" => "CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP",),
		...
		"criminal_record" => array("type" => "varchar(128)", "label" => "Antecedentes Penal", "enabled" => "1", 'position' => 41, 'notnull' => 1, "visible" => "1", "searchall" => "1",),
		"dni" => array("type" => "integer", "label" => "DNI", "enabled" => "1", 'position' => 43, 'notnull' => 1, "visible" => "1",),
		"drivers_license" => array("type" => "varchar(128)", "label" => "Licencia Conducir", "enabled" => "1", 'position' => 45, 'notnull' => 1, "visible" => "1", "searchall" => "1",),
		"service" => array("type" => "varchar(128)", "label" => "Servicio", "enabled" => "1", 'position' => 47, 'notnull' => 0, "visible" => "1", "index" => "1", "searchall" => "1",),
		"driver_contract" => array("type" => "varchar(128)", "label" => "Contrato Chofer", "enabled" => "1", 'position' => 49, 'notnull' => 1, "visible" => "1", "searchall" => "1",),
		"contract_holder" => array("type" => "varchar(128)", "label" => "Contrato Titular", "enabled" => "1", 'position' => 51, 'notnull' => 1, "visible" => "1", "searchall" => "1",),
	...
	//  */
	// public $table_element_line = 'ingreso_personales_datosline';
	...
	//  */
	// public $fk_element = 'fk_personales_datos';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Personales_Datosline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Personales_Datos', 'fk_element'=>'fk_personales_datos'));
	...
	//  */
	// protected $childtablesoncascade = array('ingreso_personales_datosdet');

	// /**
	//  * @var Personales_DatosLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('ingreso', 'personales_datos', 'read')) {
	 ...
	 /*
     * Cargar el objeto Datos padre
    public function getDatos()
    {
        $datos = new Datos($this->db);
        if ($datos->fetch($this->fk_datos) > 0) {
            return $datos;
        }
        return null;
    } */
	...
	public function create(User $user, $notrigger = 0)
	{
		...
		// Asegurar que estamos usando la tabla correcta
	...
	/**
     * Cargar objeto Datos padre
     */
    public function loadDatos()
    {
        if ($this->fk_datos > 0) {
            $this->datos = new Datos($this->db);
            $result = $this->datos->fetch($this->fk_datos);
            return $result;
        }
        return 0;
    }
    ...
    /**
     * Obtener datos del padre (propiedades comunes)
     */
    public function getCommonProperties()
    {
        if ($this->fk_datos > 0 && !is_object($this->datos)) {
            $this->loadDatos();
        }
        return $this->datos;
    }
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('ingreso', 'personales_datos', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('ingreso', 'personales_datos_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'personales_datos/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'personales_datos/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filepath = 'personales_datos/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'personales_datos/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->ingreso->dir_output.'/personales_datos/'.$oldref;
				$dirdest = $conf->ingreso->dir_output.'/personales_datos/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->ingreso->dir_output.'/personales_datos/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('ingreso','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('ingreso','ingreso_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'INGRESO_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'INGRESO_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'INGRESO_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowPersonales_Datos")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Personales_Datos").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/ingreso/personales_datos_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowPersonales_Datos");
	...
	public function getLinesArray()
	{
		...
		$objectline = new Personales_DatosLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_personales_datos:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('INGRESO_MYOBJECT_ADDON')) {
			$conf->global->INGRESO_MYOBJECT_ADDON = 'mod_personales_datos_standard';
		}

		if (getDolGlobalString('INGRESO_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('INGRESO_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('INGRESO_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/ingreso/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefPersonales_Datos $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_personales_datos';
		...
		$modelpath = "core/modules/ingreso/doc/";
	...
	/**
     * Método adicional: Cargar desde un objeto Datos
     * ESTE ES EL ÚNICO MÉTODO NUEVO QUE AÑADIMOS
     */
    public function loadFromDatos(Datos $datos)
    {
        $datos->copyCommonPropertiesTo($this);
        return $this;
    }
}
...
/**
 * Class Personales_DatosLine. You can also remove this and generate a CRUD class for lines objects.
 */
class Personales_DatosLine extends CommonObjectLine
{
	// To complete with content of an object Personales_DatosLine
	// We should have a field rowid, fk_personales_datos and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'personales_datos'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_personales_datos'
