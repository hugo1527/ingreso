...
/**
 * \file        class/vehiculo_datos.class.php
 * \ingroup     ingreso
 * \brief       This file is a CRUD class file for Vehiculo_Datos (Create/Read/Update/Delete)
 */

// Put here all includes required by your class file
//require_once DOL_DOCUMENT_ROOT.'/core/class/commonobject.class.php';
// SOLO CAMBIO AQUÍ: heredar de Datos en lugar de CommonObject
require_once DOL_DOCUMENT_ROOT.'/custom/ingreso/class/datos.class.php';
//require_once DOL_DOCUMENT_ROOT . '/societe/class/societe.class.php';
//require_once DOL_DOCUMENT_ROOT . '/product/class/product.class.php';

/**
 * Class for Vehiculo_Datos
 */
class Vehiculo_Datos extends CommonObject
//class Vehiculo_Datos extends Datos  // CAMBIO AQUÍ: extends Datos en lugar de CommonObject
{
	/**
	 ...
	 */
	public $element = 'vehiculo_datos';

	/**
	 * @var string 		Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'ingreso_vehiculo_datos';

	/**
	 * @var string 		If permission must be checkec with hasRight('ingreso', 'read') and not hasright('mymodyle', 'vehiculo_datos', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'ingreso';

	/**
	 * @var string 		String with name of icon for vehiculo_datos. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'vehiculo_datos@ingreso' if picto is file 'img/object_vehiculo_datos.png'.
	 */
	...
	public $fields = array(
		...
		"celulaverde" => array("type" => "varchar(128)", "label" => "Celula Verde", "enabled" => "1", 'position' => 61, 'notnull' => 1, "visible" => "1", "searchall" => "1",),
		"poliza" => array("type" => "varchar(128)", "label" => "Poliza", "enabled" => "1", 'position' => 63, 'notnull' => 1, "visible" => "1", "searchall" => "1",),
	...
	//  */
	// public $table_element_line = 'ingreso_vehiculo_datosline';
	...
	//  */
	// public $fk_element = 'fk_vehiculo_datos';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Vehiculo_Datosline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Vehiculo_Datos', 'fk_element'=>'fk_vehiculo_datos'));
	...
	//  */
	// protected $childtablesoncascade = array('ingreso_vehiculo_datosdet');

	// /**
	//  * @var Vehiculo_DatosLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('ingreso', 'vehiculo_datos', 'read')) {
	...
	public function create(User $user, $notrigger = 0)
	{
		...
		if ($result > 0 && isModEnabled('agenda')) {
			...
			$action->label = $langs->transnoentities("Vehiculo_Datos").": ".$datos_padre->apellido.", ".$datos_padre->nombre." (".$this->vtvitvrto.")"; // Asunto
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('ingreso', 'vehiculo_datos', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('ingreso', 'vehiculo_datos_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'vehiculo_datos/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'vehiculo_datos/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filepath = 'vehiculo_datos/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'vehiculo_datos/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->ingreso->dir_output.'/vehiculo_datos/'.$oldref;
				$dirdest = $conf->ingreso->dir_output.'/vehiculo_datos/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->ingreso->dir_output.'/vehiculo_datos/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowVehiculo_Datos")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Vehiculo_Datos").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/ingreso/vehiculo_datos_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowVehiculo_Datos");
	...
	public function getLinesArray()
	{
		...
		$objectline = new Vehiculo_DatosLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_vehiculo_datos:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('INGRESO_MYOBJECT_ADDON')) {
			$conf->global->INGRESO_MYOBJECT_ADDON = 'mod_vehiculo_datos_standard';
		}
		...
		if (getDolGlobalString('INGRESO_MYOBJECT_ADDON')) {
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefVehiculo_Datos $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_vehiculo_datos';
	...
	public function doScheduledJob()
	{
		...
		//$conf->global->SYSLOG_FILE = '/dolibarr_mydedicatedlogfile.log';
...
/**
 * Class Vehiculo_DatosLine. You can also remove this and generate a CRUD class for lines objects.
 */
class Vehiculo_DatosLine extends CommonObjectLine
{
	// To complete with content of an object Vehiculo_DatosLine
	// We should have a field rowid, fk_vehiculo_datos and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'vehiculo_datos'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_vehiculo_datos'
