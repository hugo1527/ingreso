...
/**
 *   	\file       vehiculo_datos_list.php
 *		\ingroup    ingreso
 *		\brief      List page for vehiculo_datos
 */
...
require_once __DIR__.'/class/vehiculo_datos.class.php';
// for other modules
//dol_include_once('/othermodule/class/otherobject.class.php');
// Asegurar que la ruta de documentos esté definida
...
// --- AGREGAR AQUÍ los campos del padre ---
$arrayfields['d.apellido'] = array('label'=>"Apellido", 'checked'=>1, 'position'=>31);
$arrayfields['d.nombre'] = array('label'=>"Nombre", 'checked'=>1, 'position'=>33);
// -----------------------------------------
// Extra fields
$arrayfields['documents'] = array('label'=>'Fotos', 'checked'=>1, 'position'=>100);
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('ingreso', 'vehiculo_datos', 'write');
	$permissiontodelete = $user->hasRight('ingreso', 'vehiculo_datos', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("ingreso")) {
	accessforbidden('Module ingreso not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Vehiculo_Datos';
	$objectlabel = 'Vehiculo_Datos';
	$uploaddir = $conf->ingreso->dir_output;
...
$title = $langs->trans("Vehiculo_Datoss");
//$help_url = "EN:Module_Vehiculo_Datos|FR:Module_Vehiculo_Datos_FR|ES:Módulo_Vehiculo_Datos";
$help_url = '';
$morejs = array();
$morecss = array();
...
// --- AGREGAR CAMPOS DEL PADRE ---
$sql .= ", d.apellido, d.nombre";
// ---------------------------------
...
// --- AGREGAR JOIN CON LA TABLA PADRE ---
$sql .= " LEFT JOIN ".$db->prefix()."ingreso_datos as d ON t.fk_datos = d.rowid";
// ---------------------------------------
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/ingreso/vehiculo_datos_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-ingreso page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/ingreso/vehiculo_datos_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendVehiculo_DatosRef";
$modelmail = "vehiculo_datos";
$objecttmp = new Vehiculo_Datos($db);
...
// Fields from hook
$parameters = array('arrayfields' => $arrayfields);  // <--- ESTA ES LA LÍNEA CORREGIDA
...
while ($i < $imaxinloop) {
	...
	if ($mode == 'kanban' || $mode == 'kanbangroupby') {
		...
		// --- AGREGAR AQUÍ para mostrar campos del padre ---
		if (!empty($arrayfields['d.apellido']['checked'])) {
			print '<td class="nowrap">'.dol_escape_htmltag($obj->apellido).'</td>';
		}
		if (!empty($arrayfields['d.nombre']['checked'])) {
			print '<td class="nowrap">'.dol_escape_htmltag($obj->nombre).'</td>';
		}
		// ------------------------------------------------
		 // --- AGREGAR AQUÍ LA CELDA DE DOCUMENTOS ---
        /*if (!empty($arrayfields['documents']['checked'])) {
            print '<td class="center nowraponall">';
            
            // Crear objeto temporal para contar archivos
            $tmpobject = new Vehiculo_Datos($db);
            $tmpobject->id = $obj->rowid;
            $tmpobject->ref = $obj->ref; // Asumiendo que tienes campo ref
            
            // Contar documentos
            $nbfiles = $tmpobject->countOfFiles();
            
            if ($nbfiles > 0) {
                // Enlace a la pestaña de documentos
                $url = dol_buildpath('/ingreso/vehiculo_datos_card.php', 1).'?id='.$obj->rowid.'#documents';
                print '<a href="'.$url.'" title="'.dol_escape_htmltag($langs->trans("ViewDocuments")).'">';
                print img_picto('', 'object_gliph.png@ingreso'); // O usa: 'fa-file-image-o'
                print ' <span class="badge">'.$nbfiles.'</span>';
                print '</a>';
            } else {
		...
		// --- COLUMNA DE DOCUMENTOS - SOLUCIÓN SIMPLE DIRECTA ---
		if (!empty($arrayfields['documents']['checked'])) {
			print '<td class="center">';
			
			// Determinar la referencia del objeto (usar rowid si ref no existe)
			$object_ref = isset($obj->ref) ? $obj->ref : $obj->rowid;
			
			// Contar archivos directamente
			$dir = $conf->ingreso->dir_output.'/vehiculo_datos/'.dol_sanitizeFileName($object_ref);
			$nbfiles = 0;
			
			if (is_dir($dir)) {
				// Usar parámetros más simples para dol_dir_list
				$files = dol_dir_list($dir, 'files');
				$nbfiles = count($files);
			}
			...
			$url = dol_buildpath('/ingreso/vehiculo_datos_card.php', 1).'?id='.$obj->rowid.'#documents';
			if ($nbfiles > 0) {
				print '<a href="'.$url.'" class="classfortooltip" title="Ver '.$nbfiles.' documento(s)">';
				print img_picto('', 'object_gliph.png@ingreso', 'class="pictofixedwidth"');
				print ' <span class="badge">'.$nbfiles.'</span>';
				print '</a>';
			} else {
				print '<a href="'.$url.'" title="Agregar documentos">';
				print img_picto('', 'object_gliph.png@ingreso', 'class="pictofixedwidth opacitymedium"');
				print '</a>';
			}
		...
		// Fields from hook
		//$parameters = array('arrayfields' =>, 'object' => $object, 'obj' => $obj, 'i' => $i, 'totalarray' => &$totalarray);
...
//$parameters = array('arrayfields' =>, 'sql' => $sql);
